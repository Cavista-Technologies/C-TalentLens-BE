...
public class DashboardService(TalentLensDbContext dbContext, IClock clock) : IDashboardService
{
    ...
    {
        var scopedQuery = accessScope.ApplyTo(dbContext.Requisitions);
        var overview = await BuildOverviewAsync(scopedQuery, cancellationToken);
        var pipelineCounts = await scopedQuery
            .Where(requisition => requisition.CurrentStatus != RequisitionStatus.Closed)
            .GroupBy(requisition => requisition.CurrentStage)
            .Select(group => new { Stage = group.Key, Count = group.Count() })
            .ToDictionaryAsync(item => item.Stage, item => item.Count, cancellationToken);

        var scoped = await scopedQuery
            .AsNoTracking()
        ...
        var pipeline = BuildPipelineDashboard(scoped, pipelineCounts);
    ...
    private async Task<RecruitmentOverviewResponse> BuildOverviewAsync(
        IQueryable<Requisition> requisitions,
        CancellationToken cancellationToken)
    {
        var rows = await requisitions
            .GroupBy(_ => 1)
            .Select(group => new
            {
                Active = group.Count(requisition => requisition.CurrentStatus != RequisitionStatus.Closed),
                Closed = group.Count(requisition => requisition.CurrentStatus == RequisitionStatus.Closed),
                HiringGoal = group.Sum(requisition => requisition.HiringGoal),
                FilledGoal = group.Sum(requisition => requisition.FilledGoal)
            })
            .FirstOrDefaultAsync(cancellationToken);

        if (rows is null)
        {
            return new RecruitmentOverviewResponse(0, 0, 0, 0, 0);
        }
        ...
        return new RecruitmentOverviewResponse(
            ...
            rows.HiringGoal,
            rows.FilledGoal,
            Math.Max(rows.HiringGoal - rows.FilledGoal, 0));
    }

    private PipelineDashboardResponse BuildPipelineDashboard(
        IReadOnlyCollection<Requisition> requisitions,
        IReadOnlyDictionary<PipelineStage, int> stageCounts)
    {
        ...
        var stageMetrics = trackedStages
            .Select(stage => new PipelineStageMetricResponse(
                stage,
                GetStageCount(stageCounts, stage),
                requisitions
        ...
        return new PipelineDashboardResponse(
            GetStageCount(stageCounts, PipelineStage.JobPosting),
            GetStageCount(stageCounts, PipelineStage.PipeliningSourcing),
            GetStageCount(stageCounts, PipelineStage.SparkHire),
            GetStageCount(stageCounts, PipelineStage.Interview),
            GetStageCount(stageCounts, PipelineStage.RequestToHire),
            GetStageCount(stageCounts, PipelineStage.OfferedHired),
    ...
    private static int GetStageCount(IReadOnlyDictionary<PipelineStage, int> stageCounts, PipelineStage stage)
    {
        return stageCounts.GetValueOrDefault(stage);
    }
