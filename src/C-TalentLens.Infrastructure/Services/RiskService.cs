    ...
    IRiskScoringEngine riskScoringEngine) : IRiskService
{
    ...
    {
        var requisitions = await accessScope.ApplyTo(Query())
    ...
    {
        var requisitions = await ApplyQuery(accessScope.ApplyTo(Query()), query)
            .Where(requisition => requisition.CurrentStatus != RequisitionStatus.Closed)
            .ToListAsync(cancellationToken);
    ...
    private static IQueryable<Requisition> ApplyQuery(IQueryable<Requisition> query, RiskDashboardQuery request)
    {
        ...
        if (request.OpenedFrom is not null)
        {
            query = query.Where(requisition => requisition.DateOpened >= request.OpenedFrom);
        }

        if (request.OpenedTo is not null)
        {
            query = query.Where(requisition => requisition.DateOpened <= request.OpenedTo);
        }
